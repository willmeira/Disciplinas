---
title: "Análise de Série Temporais - Trabalho 02"
subtitle: "Prof Fernando Lucambio Pérez"
...
No gráfico acima notamos a correlação dos erros, também na função de autocorrelação parcial (PACF) a correlação autoregressiva. 
...
Consideramos o modelo bem ajustado, sendo que o os resíduos estão dentro dos limites do intervalor de confiança.
...
```
  ...
  O gráfico apresenta apenas o final da série, pois esse período possui os dados mais importantes do o ajuste.


## Exercício 18

Ajustando um modelo ***AR(2)*** para a série de mortalidade cardiovascular ***cmort*** discutida no exercício anterior:
...
b) Valores baixos para as estimativas dos erros padrões nos dois modelos. Foram também bem próximas nos dois modelos
concordando com a proposição III.10, que refere-se aos estimadores do modelo ARIMA para amostras grandes. 
  Em outras palavras, muito provávelmente a distribuição dos estimadores para o modelo ARIMA está próxima 
...
Como o modelo depende somente dos dados amostrais, e as componentes de média móvel e autoregressivas dependem apenas das
últimas realizações em cada modelo simulado, os resultados são diferentes em cada série. 
...
Os resultados encontrados sugerem três tipos de comportamento para as séries 
