...
mod_gb2_sig <- refit(mod_gb2_sig)
plot(mod_gb2_sig)
wp(mod_gb2_sig)
GAIC(mod_lm, mod_exp, mod_ga, mod_gb2, mod_gb2_sig, k=2)

mod_gb2_signu <- update(mod_gb2_sig, nu.fo = Radiation ~ Temperature + Pressure + Speed + WindDirection + Month + TimeHour, 
                        data = dados, method=RS(),c.crit=0.1)
mod_gb2_signu <- refit(mod_gb2_signu)
plot(mod_gb2_signu)
wp(mod_gb2_signu, ylim.all = 0.4)
GAIC(mod_lm, mod_exp, mod_ga, mod_gb2, mod_gb2_sig, mod_gb2_signu, k=2)


mod_gb2_signutau <- update(mod_gb2_signu, tau.fo = Radiation ~ Temperature + Pressure + Speed + WindDirection + Month + TimeHour, 
                        data = dados, method=RS(),c.crit=0.1)
mod_gb2_signutau <- refit(mod_gb2_signutau)
plot(mod_gb2_signutau)
wp(mod_gb2_signutau, ylim.all = 0.4)
GAIC(mod_lm, mod_exp, mod_ga, mod_gb2, mod_gb2_sig, mod_gb2_signu, mod_gb2_signutau, k=2)
