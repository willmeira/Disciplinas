...
#*********************************
dados <- read_csv("C:/Users/Simone/Desktop/base_vivo_morto.csv")
...
par(mfrow = c(3,3))
hist(dados$dead,breaks = 2,main = "0 morreu 1 sobreviveu " ,xlab = "sobreviveu" )
hist(dados$abfunc, breaks = 2,main = "0 NÃO 1 SIM " ,xlab =  "Airbag Funcionou")
hist(dados$cinto, breaks = 2,main = "0 NÃO 1 SIM" ,xlab =  "Possui Cinto")
hist(dados$frontal, breaks = 2,main = "0 NÃO 1 SIM" ,xlab =  "Acidente frontal ")
hist(dados$sexo,breaks = 2,main = "1 HOMEM 0 MULHER" ,xlab =  "Sexo")
hist(dados$ocupantes,breaks = 2,main = "1 SIM 0 NÃO" ,xlab =  "2 passageiros na frente")
```
...
```{r,echo=FALSE, eval=TRUE, results="hide"}
g1<-ggplot(dados, aes(x=factor(sobrev), y=veloc, color=factor(sobrev))) +  geom_boxplot()+ xlab('Velocidade')+ ylab('') + theme(legend.title=element_blank())

g5<-ggplot(dados, aes(x=factor(sobrev), y=idade, color=factor(sobrev))) +  geom_boxplot()+ xlab('Idade')+ ylab('') + theme(legend.title=element_blank())
